...
export async function GET() {
  try {
    ...
    return NextResponse.json({ watchlist: items.map(serializeLibraryItem) });
  } catch (error) {
...
export async function POST(request: Request) {
  try {
    ...
    if (!parsed.success) return badRequest("Invalid watchlist item.");
    ...
    return NextResponse.json({ watchlistItem: serializeLibraryItem(item) });
  } catch (error) {
    return serverError(error);
  }
}
...
export async function DELETE(request: Request) {
  try {
    ...
    await prisma.watchListItem.deleteMany({ where: { userId: user.id, mediaId } });
